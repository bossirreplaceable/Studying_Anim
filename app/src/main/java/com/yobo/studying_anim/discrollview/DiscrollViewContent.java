package com.yobo.studying_anim.discrollview;
...
public class DiscrollViewContent extends LinearLayout {
	...
	public static class MyLayoutParams extends LinearLayout.LayoutParams{
		int mDiscrollveFromBgColor;//背景颜色变化开始值
		int mDiscrollveToBgColor;//背景颜色变化结束值
		boolean mDiscrollveAlpha;//是否需要透明度动画
		int mDisCrollveTranslation;//平移值
		boolean mDiscrollveScaleX;//是否需要x轴方向缩放
		boolean mDiscrollveScaleY;//是否需要y轴方向缩放

		MyLayoutParams(Context context, AttributeSet attrs) {
			super(context, attrs);
			// 从child里面拿到我自定义的属性
			@SuppressLint("CustomViewStyleable") TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.DiscrollView_LayoutParams);
